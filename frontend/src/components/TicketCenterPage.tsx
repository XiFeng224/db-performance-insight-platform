...
import { Alert, Button, Card, Col, Drawer, Form, Input, InputNumber, Modal, Rate, Row, Select, Space, Statistic, Table, Tag, Timeline, Typography, message, Segmented } from 'antd';
...
const TicketCenterPage: React.FC = () => {
  ...
  const filteredTickets = useMemo(() => {
    ...
    return tickets.filter((t) => {
      const isDrill = (t.description || '').includes('[DRILL_MODE]');
      if (drillOnly && !isDrill) return false;
      if (!kw) return true;
      const text = `${t.id} ${t.title} ${t.location || ''} ${t.description || ''}`.toLowerCase();
      return text.includes(kw);
    });
  }, [tickets, keyword, drillOnly]);
    ...
    <div className="fade-in">
          ...
          <Card>
            <Space style={{ marginBottom: 12, width: '100%', justifyContent: 'space-between' }} wrap>
              <Space wrap>
                ...
                <Input
                  allowClear
                  placeholder="搜索工单ID/标题/地点"
                  value={keyword}
                  onChange={(e) => setKeyword(e.target.value)}
                  style={{ width: 220 }}
                />
                <Button type={drillOnly ? 'primary' : 'default'} onClick={() => setDrillOnly((v) => !v)}>
                  {drillOnly ? '仅看演练：开启' : '仅看演练'}
                </Button>
              </Space>
      ...
      <Drawer title={selected ? `工单详情 #${selected.id}` : '工单详情'} open={detailOpen} onClose={() => setDetailOpen(false)} width={560}>
        {selected && (
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            <Card size="small" title="处理时间线（责任链）">
              <Timeline
                items={events.map((e) => ({
                  color: e.event_type === 'assigned' ? 'blue' : e.event_type === 'status_changed' ? 'green' : e.event_type === 'sla_escalated' ? 'red' : 'gray',
                  children: `${e.created_at} - [${e.event_type}] ${e.content}`,
                }))}
              />
            </Card>
          </Space>
      ...
      >
        <Form
          ...
          onFinish={async (values) => {
            ...
            try {
              ...
              message.error('关闭失败，请检查后端服务或稍后重试');
            } finally {
        ...
        >
          ...
          <Form.Item>
            <Button type="primary" htmlType="submit" loading={closeSubmitting}>确认关闭并回填</Button>
          </Form.Item>
        </Form>
      </Modal>
