  ...
  const handleCompare = useCallback(async () => {
    ...
    try {
      const response = await axios.get<ComparisonData>(
        'http://localhost:8000/api/comparison/metrics',
        {
          params: {
        ...
        <>
          <Card style={{ marginBottom: 16 }}>
            <Space wrap>
              ...
              <Tag color="blue">{comparison.data_source || 'unknown'}</Tag>
              <Text type="secondary">状态：</Text>
              <Tag color={comparison.degraded ? 'orange' : 'green'}>{comparison.degraded ? 'degraded' : 'normal'}</Tag>
